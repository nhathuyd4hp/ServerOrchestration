...
class SharePoint(IWebDriver):
    ...
    def search(self,site_url:str, keyword:int | str | None = None) -> Tuple[str,pd.DataFrame]:
        ...
        if keyword:
            ...
            time.sleep(self.retry_interval)
            self.wait.until(
                EC.element_to_be_clickable((By.CSS_SELECTOR,"button[title='Search']"))
            ).click()
            time.sleep(self.retry_interval)
            WebDriverWait(
                driver=self.browser,
                ...
                EC.element_to_be_clickable((
                    By.XPATH,
                    f"//button[@title='Clear filter' and contains(normalize-space(.), '{keyword}')]"
                ))
            )
            # 
            time.sleep(10)
            ...
            columns = [column.get_attribute("title") if column.get_attribute("title") else re.sub(r'[\ue000-\uf8ff]|\n|Press C to open file hover card', '', column.text) for column in columns]
            rows = self.wait.until(
                method = EC.any_of(
                    EC.presence_of_all_elements_located((By.CSS_SELECTOR, "div[role='presentation'][data-automationid='ListCell']")),
                    EC.presence_of_all_elements_located((By.CSS_SELECTOR, "div[id^='virtualized-list'][role='row']")),
                ),
                message="Không tìm thấy dữ liệu"
            )
            for row in rows:
                cells = row.find_elements(By.CSS_SELECTOR,"div[role='gridcell'], div[data-automationid='field-InternalAddColumn']")
                if [cell.text for cell in cells][columns.index("Type")] == '':
                    ...
                    self.wait.until(
                        EC.presence_of_element_located((
                            By.XPATH,
                            f"//div[@type='button' and @data-automationid='breadcrumb-crumb' and normalize-space(.)='{Folder}']"
                        ))
                    )
                    break
            self.wait.until(
        ...
        for step in file:
            ...
            for gridcell in gridcells:
                ...
                if re.match(step,text):
                    ...
                    self.wait.until(
                        EC.presence_of_element_located(
                            (By.XPATH, f'//div[@type="button" and @data-automationid="breadcrumb-crumb" and text()="{step}"]')
                        )
                    )
                    break
            time.sleep(self.retry_interval)  
